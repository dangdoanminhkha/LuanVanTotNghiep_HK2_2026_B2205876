...
const AddressManagement = () => {
  ...
  const handleDelete = async (id) => {
    showWarning('Xóa địa chỉ', 'Bạn có chắc chắn muốn xóa địa chỉ này?', async () => {
      try {
        ...
        showError('Lỗi', 'Không thể xóa địa chỉ này');
      }
    }, true);
  };
                ...
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                  ...
                  <div className="flex items-center gap-3 sm:flex-col sm:items-end sm:gap-2">
                    <div className="flex gap-3 text-sm">
                      ...
                      {!addr.is_default && (
                        <button onClick={() => handleDelete(addr.id)} className="text-red-600 hover:text-red-800 font-medium">Xóa</button>
                      )}
                    </div>
                    {!addr.is_default && (
