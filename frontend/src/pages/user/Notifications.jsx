...
const Notifications = () => {
  ...
  const markAllAsRead = async () => {
    try {
      ...
      // Refetch để đảm bảo trạng thái đã đọc đồng bộ tuyệt đối với backend
  ...
  const mapNotificationToOrderTab = (notification) => {
    const text = `${notification?.title || ''} ${notification?.message || ''}`.toLowerCase();

    if (text.includes('hủy') || text.includes('huỷ')) {
      return { initialGroup: 'order', initialFilter: 'cancelled' };
    }
    if (text.includes('hoàn tiền')) {
      return { initialGroup: 'return', initialFilter: 'refund' };
    }
    if (text.includes('bị từ chối') && text.includes('hoàn trả')) {
      return { initialGroup: 'return', initialFilter: 'return_rejected' };
    }
    if (text.includes('chấp nhận') && text.includes('hoàn trả')) {
      return { initialGroup: 'return', initialFilter: 'return_approved' };
    }
    if (text.includes('đã gửi hàng hoàn')) {
      return { initialGroup: 'return', initialFilter: 'return_shipped' };
    }
    if (text.includes('đã nhận hàng hoàn')) {
      return { initialGroup: 'return', initialFilter: 'return_received' };
    }

    return { initialGroup: 'order', initialFilter: 'all' };
  };
  ...
  const handleNotificationClick = async (notification) => {
    ...
    if (notification.type === 'order_status') {
      const tabState = mapNotificationToOrderTab(notification);
      navigate('/account/orders', { state: tabState });
