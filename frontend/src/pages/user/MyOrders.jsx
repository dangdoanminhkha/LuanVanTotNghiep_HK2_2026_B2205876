...
const MyOrders = () => {
  ...
  const [shippingForm, setShippingForm] = useState({
    carrier: '', tracking_code: '', receipt_image: '', refund_info: ''
  });
  ...
  useEffect(() => {
    if (location.state?.initialGroup) {
      setActiveGroup(location.state.initialGroup);
    }
    if (location.state?.initialFilter) {
      setActiveFilter(location.state.initialFilter);
    }
  }, [location.state?.initialGroup, location.state?.initialFilter]);
  ...
  const RETURN_STATUSES = ['return_requested', 'return_rejected', 'return_approved', 'return_shipped', 'return_received', 'refund'];
  ...
  const handleConfirmShipped = async () => {
    if (!shippingForm.carrier || !shippingForm.tracking_code) {
      showError('Lỗi', 'Vui lòng nhập đơn vị vận chuyển và mã vận đơn');
      return;
    }
    setShippingSubmitting(true);
    try {
      await api.post(`/orders/${shippingModal}/confirm-return-shipped`, shippingForm);
      showSuccess('Thành công', 'Đã xác nhận thông tin gửi hàng hoàn!');
      setShippingModal(null);
      fetchOrders();
    } catch (err) {
      showError('Lỗi', err.response?.data?.error || 'Có lỗi xảy ra khi xác nhận');
    } finally {
  ...
  const returnStatusTabs = [
    ...
    { key: 'return_shipped', label: 'Đã gửi hàng', statuses: ['return_shipped'] },
  ...
  const getStatusBadge = (status) => {
    const statusMap = {
      ...
      return_shipped: { text: '📤 Đã gửi hàng hoàn', color: 'bg-indigo-100 text-indigo-800' },
  ...
  return (
    <div className="space-y-4">
                  ...
                  {selectedOrder === order.id && (
                    <div className="mt-6 pt-6 border-t space-y-3">
                      <div className="grid md:grid-cols-2 gap-4">
                        ...
                        <p className="text-gray-600">
                          {[order.address_detail, order.ward_name, order.district_name, order.province_name].filter(Boolean).join(', ') || order.shipping_address || 'Chưa cập nhật'}
                        </p>
                        </div>
                        <div>
                      ...
                      <div className="mt-6 pt-6 border-t">
                        {order.status === 'return_shipped' && order.return_detailed && (
                          <div className="mb-6 p-4 bg-indigo-50 rounded-lg border border-indigo-100">
                            <h4 className="font-semibold text-indigo-800 mb-3 flex items-center gap-2">
                              📤 Thông tin gửi hàng hoàn của bạn
                            </h4>
                            {(() => {
                              let details = order.return_detailed;
                              ...
                              return (
                                <div className="grid md:grid-cols-2 gap-4 text-sm text-indigo-900">
                                  <div>
                                    <p><span className="text-indigo-600 font-medium">ĐVVC:</span> {info.carrier}</p>
                                    <p><span className="text-indigo-600 font-medium">Mã vận đơn:</span> {info.tracking_code}</p>
                                  </div>
                                  <div>
                                    <p><span className="text-indigo-600 font-medium">TT Hoàn tiền:</span> {info.refund_info || 'Không có'}</p>
                                    {info.receipt_image && (
                                      <div className="mt-2">
                                        <p className="text-indigo-600 font-medium mb-1">Ảnh biên nhận:</p>
                                        <a href={normalizeImageUrl(info.receipt_image)} target="_blank" rel="noopener noreferrer">
                                          <img src={normalizeImageUrl(info.receipt_image)} className="w-16 h-16 object-cover rounded border border-indigo-200" alt="Biên nhận" />
                                        </a>
                                      </div>
                                    )}
                                  </div>
                ...
                <div className="bg-gray-50 px-4 py-3 border-t flex flex-wrap justify-between items-center gap-3">
                  <div className="flex gap-3 items-center flex-wrap">
                    ...
                    {order.status === 'return_approved' && (
                      <button
                        onClick={() => {
                          setShippingForm({ carrier: '', tracking_code: '', receipt_image: '', refund_info: '' });
                          setShippingModal(order.id);
                        }}
                        className="text-blue-600 hover:text-blue-700 font-medium text-sm border border-blue-300 px-3 py-1 rounded-lg hover:bg-blue-50"
                      >
                        📤 Xác nhận đã gửi hàng hoàn
                      </button>
                    )}
      ...
      {/* Modal Xác nhận gửi hàng hoàn */}
      {shippingModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg">
            <div className="flex justify-between items-center px-6 py-4 border-b">
              <h3 className="text-lg font-bold text-indigo-900">📤 Cung cấp thông tin gửi hàng</h3>
              <button onClick={() => setShippingModal(null)} className="text-gray-400 hover:text-gray-600 text-2xl leading-none">&times;</button>
            </div>
            <div className="px-6 py-5 space-y-4">
              <p className="text-sm text-gray-600 mb-2">Vui lòng cung cấp mã vận đơn để cửa hàng tiện theo dõi kiện hàng bạn gửi lại.</p>
              
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Đơn vị vận chuyển <span className="text-red-500">*</span></label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Mã vận đơn (Tracking Code) <span className="text-red-500">*</span></label>
                <input
                  type="text"
                  value={shippingForm.tracking_code}
                  onChange={e => setShippingForm({...shippingForm, tracking_code: e.target.value})}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Thông tin nhận hoàn tiền (Ngân hàng)</label>
                <input
                  type="text"
                  value={shippingForm.refund_info}
                  onChange={e => setShippingForm({...shippingForm, refund_info: e.target.value})}
                  placeholder="VD: Vietcombank - 123456789 - NGUYEN VAN A"
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400"
                />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Ảnh biên nhận gửi hàng (Tuỳ chọn)</label>
                <FileUpload 
                  onUploadSuccess={(url) => setShippingForm({...shippingForm, receipt_image: url})}
                  currentImage={shippingForm.receipt_image}
                  folder="returns"
                />
              </div>
            </div>
            <div className="px-6 pb-5 flex justify-end gap-3 border-t pt-4">
              <button onClick={() => setShippingModal(null)} className="px-4 py-2 text-sm border rounded-lg hover:bg-gray-50">Hủy</button>
              <button
                onClick={handleConfirmShipped}
                disabled={shippingSubmitting}
                className="px-4 py-2 text-sm bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-50"
              >
                {shippingSubmitting ? 'Đang gửi...' : 'Xác nhận đã gửi'}
              </button>
            </div>
          </div>
        </div>
